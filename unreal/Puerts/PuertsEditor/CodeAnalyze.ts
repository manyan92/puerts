...
var Global_TSROOT = "TypeScripts"



function WarnMessage(filePath, Message) {
    console.warn(Message);
    UE.TSCompilerErrorReportSubsystem.AddErrorReport(filePath, UE.ETSCompileErrorLevel.Warn, Message);
}
...
function getCustomSystem(): ts.System {
    ...
    function getExecutingFilePath(): string {
        return getCurrentDirectory() + "Content/PuertsDependencies/PuertsEditor/node_modules/typescript/lib/tsc.js";
    }
...
function logErrors(allDiagnostics: readonly ts.Diagnostic[]) {
    allDiagnostics.forEach(diagnostic => {
        ...
        if (diagnostic.file) {
            ...
            let ErrorStr = `  Error ${diagnostic.file.fileName} (${line + 1},${character + 1}): ${message}`
            console.error(ErrorStr);
            UE.TSCompilerErrorReportSubsystem.AddErrorReport(diagnostic.file.fileName, UE.ETSCompileErrorLevel.Error, ErrorStr);
        } else {
...
function readAndParseConfigFile(configFilePath: string, InfoListOfPlugins: UE.TArray<UE.PuretsPluginInfo>): ts.ParsedCommandLine {
    let readResult = ts.readConfigFile(configFilePath, customSystem.readFile);

    let ListOfInclude: Array<string> = new Array;

    let TSPath = Global_TSROOT + "/**/*";

    ListOfInclude.push("./" + TSPath);

    for (var i = 0; i < InfoListOfPlugins.Num(); i++) {
        let absPath = InfoListOfPlugins.Get(i).Path + "/" + TSPath;
        if (absPath.startsWith(customSystem.getCurrentDirectory())) {
            let relativePath = absPath.substr(customSystem.getCurrentDirectory().length)
            ListOfInclude.push("./" + relativePath)
        }
    }

    readResult.config["include"] = ListOfInclude;
    readResult.config["compilerOptions"]["outDir"] = customSystem.getCurrentDirectory()
    readResult.config["compilerOptions"]["baseUrl"] = customSystem.getCurrentDirectory()

    //console.log("exclude:" + readResult.config["exclude"]);

    if (readResult.config["exclude"] == undefined) {
        //console.log("exclude undefined")
        readResult.config["exclude"] = []
    }
...
function watch(configFilePath: string, InfoListOfPlugins: UE.TArray<UE.PuretsPluginInfo>) {

    const versionsFilePath = customSystem.getCurrentDirectory() + "ts_file_versions_info.json";

    let { fileNames, options } = readAndParseConfigFile(configFilePath, InfoListOfPlugins);
    ...
    function DoErrorReport() {
        Object.keys(fileVersions).forEach(filename => {
            if (UE.TSCompilerErrorReportSubsystem.HasCompilerError(filename)) {
                if (fileVersions[filename].version != "WithError") {
                    fileVersions[filename].version = "WithError"
                }
            }
        })

        UE.TSCompilerErrorReportSubsystem.ShowErrorReport();
    }
    ...
    if (diagnostics.length > 0) {
        ...
        fileNames.forEach(fileName => {
            if (!(fileName in restoredFileVersions) || restoredFileVersions[fileName].version != fileVersions[fileName].version || !restoredFileVersions[fileName].processed) {
                ...
                onSourceFileAddOrChange(fileName, false, program, true);
                changed = true;
            } else {
        ...
        if (versionsFileExisted) {
            fileNames.forEach(fileName => {
                ...
                let BPExisted = false;
                BPExisted = UE.PEBlueprintAsset.Existed(fileName);
                if (restoredFileVersions[fileName].processed && !BPExisted) {
                    console.warn("lost file blueprints:" + fileName);
    ...
    dirWatcher.OnChanged.Add((added, modified, removed) => {
        setTimeout(() => {
            ...
            for (let i = 0; i < removed.Num(); i++) {
                if (removed.Get(i) == versionsFilePath) {
                    console.log("Version file change!");
                    Object.keys(fileVersions).forEach(filename => {
            ...
            modifiedFiles.forEach(fileName => {
                if (fileName in fileVersions) {
                    UE.TSCompilerErrorReportSubsystem.CleanupErrorReportFile(fileName);
                    let md5 = UE.FileSystemOperation.FileMD5Hash(fileName);
    ...
    function onFileAdded(): void {
        let cmdLine = readAndParseConfigFile(configFilePath, InfoListOfPlugins);
        ...
        if (sourceFile) {
            ...
            if (diagnostics.length > 0) {
                ...
                if (doEmitBP) {
                    ...
                    console.warn(`${sourceFilePath} not do Emit BP.`)
                }
                ...
                if (!sourceFile.isDeclarationFile) {
                    ...
                    if (!emitOutput.emitSkipped) {
                        ...
                        emitOutput.outputFiles.forEach(output => {
                            ...
                            if (output.name.indexOf(Global_TSROOT) <= 0) {
                                WarnMessage(sourceFilePath, `${output.name} not in a typescripts path.`)
                                return;
                            }

                            if (doEmitJs) {
                                let OutputPath = output.name.replace(Global_TSROOT, "Content/JavaScript");
                                console.log(`write ${OutputPath} ...`)
                                if (OutputPath.endsWith(".js.map")) {
                                    let MapObject = JSON.parse(output.text);
                                    MapObject["sources"][0] = output.name.replace(".js.map", ".ts")
                                    output.text = JSON.stringify(MapObject);
                                }
                                UE.FileSystemOperation.WriteFile(OutputPath, output.text);
                            }

                            if (output.name.endsWith(".js") || output.name.endsWith(".mjs")) {
                                jsSource = output.text;
                                for (let index = 0; index < InfoListOfPlugins.Num(); index++) {
                                    const plugin: UE.PuretsPluginInfo = InfoListOfPlugins.Get(index);
                                    //console.log(output.name + "," + plugin.Path)

                                    if (output.name.startsWith(plugin.Path)) {
                                        GameModule = "@" + plugin.PluginName
                                        moduleFileName = output.name.substr(plugin.Path.length + Global_TSROOT.length + 2);
                                        modulePath = tsi.getDirectoryPath(moduleFileName);
                                        console.log(`modulePath:${modulePath} moduleFileName:${moduleFileName}`);
                                        moduleFileName = tsi.removeExtension(moduleFileName, ".js");
                                    }
                                }

                                if (moduleFileName == undefined) {
                                    moduleFileName = output.name.substr(options.outDir.length + Global_TSROOT.length + 1);
                                    modulePath = tsi.getDirectoryPath(moduleFileName);
                                    moduleFileName = tsi.removeExtension(moduleFileName, output.name.endsWith(".js") ? ".js" : ".mjs");
                                }

                                console.log(`FileChange modulePath：${modulePath} moduleFileName:${moduleFileName} sourceFileName:${output.name} options.outDir:${options.outDir}`)

                            }
                        });
                        if (moduleFileName && reload) {
                            UE.FileSystemOperation.PuertsNotifyChange(GameModule + "/" + moduleFileName, jsSource);
                        }
                        ...
                        ts.forEachChild(sourceFile, (node) => {
                            if (ts.isExportAssignment(node) && ts.isIdentifier(node.expression)) {
                                ...
                                if (type.getSymbol().getName() != tsi.getBaseFileName(moduleFileName)) {
                                    WarnMessage(sourceFilePath, `type name must the same as file name! ${type.getSymbol().getName()} & ${tsi.getBaseFileName(moduleFileName)}`);
                                    return;
                                }
                                ...
                                if (!structOfType) {
                                    console.log(`${sourceFile} not a UClass!`)
                                    return
                                }
                                ...
                                if (structOfType.GetClass().IsChildOf(UE.Class.StaticClass())) {
                                    ...
                                    WarnMessage(sourceFilePath, "do not support UStruct:" + checker.typeToString(type));
                                    return;
                                }
                                ...
                                if (baseTypeUClass) {
                                    if (isSubclassOf(type, "Subsystem")) {
                                        ...
                                        WarnMessage(sourceFilePath, "do not support Subsystem " + checker.typeToString(type));
                                        return;
                                    }
                                    ...
                                    WarnMessage(sourceFilePath, "can not find base for " + checker.typeToString(type));
                                }
                            }
                        });
                        ...
                        if (foundType && foundBaseTypeUClass) {
                            ...
                            pendingBlueprintRefleshJobs.push({ type: foundType, op: () => onBlueprintTypeAddOrChange(foundBaseTypeUClass, foundType, GameModule, modulePath, sourceFilePath) });
                        }
                    }
                }
            }
            ...
            function getUClassOfType(type: ts.Type): UE.Object {
                ...
                if (moduleNames.length > 0 && moduleNames[0] == 'ue') {
                    ...
                    if (options.outDir && sourceFileName.startsWith(options.outDir)) {
                        if (sourceFileName.indexOf(Global_TSROOT) <= 0) {
                            return;
                        }
                        ...
                        for (let index = 0; index < InfoListOfPlugins.Num(); index++) {
                            const plugin: UE.PuretsPluginInfo = InfoListOfPlugins.Get(index);
                            if (sourceFileName.startsWith(plugin.Path)) {
                                GameModule = "@" + plugin.PluginName
                                moduleFileName = sourceFileName.substr(plugin.Path.length + Global_TSROOT.length + 2);
                                modulePath = tsi.getDirectoryPath(moduleFileName);
                                console.log(`modulePath:${modulePath} moduleFileName:${moduleFileName}`);
                                moduleFileName = tsi.removeExtension(moduleFileName, ".js");
                            }
                        ...
                        if (moduleFileName == undefined) {
                            moduleFileName = sourceFileName.substr(options.outDir.length + Global_TSROOT.length + 1);
                            modulePath = tsi.getDirectoryPath(moduleFileName);
                            moduleFileName = tsi.removeExtension(moduleFileName, sourceFileName.endsWith(".js") ? ".js" : ".mjs");
                        }

                        console.log(`2 + modulePath：${modulePath} moduleFileName:${moduleFileName} sourceFileName:${sourceFileName} options.outDir:${options.outDir}`)

                        let bp = new UE.PEBlueprintAsset();
                        bp.LoadOrCreate(GameModule, type.getSymbol().getName(), modulePath, baseTypeUClass as UE.Class, 0, 0);
                        bp.Save();
                    ...
                    if (ts.isTypeReferenceNode(typeNode) && type.symbol) {
                        ...
                        if (!typeNode.typeArguments || typeNode.typeArguments.length == 0) {
                            ...
                            if (!uclass) {
                                ...
                                WarnMessage(sourceFilePath, "can not find type of " + typeName);
                                return undefined;
                            }
                            ...
                            if (!typeArguments) {
                                WarnMessage(sourceFilePath, "can not find type arguments of " + node.getFullText());
                                return undefined;
                            }
                            ...
                            if (!result || result.pinType.PinContainerType != UE.EPinContainerType.None && typeName != '$Ref' && typeName != '$InRef') {
                                WarnMessage(sourceFilePath, "can not find pin type of typeArguments[0] " + typeName);
                                return undefined;
                            }
                            ...
                            if (typeName == 'TArray' || typeName == 'TSet') {
                                ...
                                if (!valuePinType || valuePinType.pinType.PinContainerType != UE.EPinContainerType.None) {
                                    WarnMessage(sourceFilePath, "can not find pin type of typeArguments[1] " + typeName);
                                    return undefined;
                                }
                                if (children[2]) {
                                ...
                                WarnMessage(sourceFilePath, "not support generic type: " + typeName);
                                return undefined;
                            }
                        }
                    } else {
                        ...
                        switch (typeNode.kind) {
                            ...
                            case ts.SyntaxKind.BooleanKeyword:
                                ...
                                break;
                            default:
                                WarnMessage(sourceFilePath, "not support kind: " + typeNode.kind);
                                return undefined;
                        }
                        let pinType = new UE.PEGraphPinType(category, undefined, UE.EPinContainerType.None, false, false);
                        return { pinType: pinType };
            ...
            function onBlueprintTypeAddOrChange(baseTypeUClass: UE.Class, type: ts.Type, GameModule: string, modulePath: string, sourceFilePath: string) {
                console.log(`gen blueprint for ${type.getSymbol().getName()},module:${GameModule} path: ${modulePath}`);
                let lsFunctionLibrary: boolean = baseTypeUClass && baseTypeUClass.GetName() === "BlueprintFunctionLibrary";
                let bp = new UE.PEBlueprintAsset();
                bp.LoadOrCreateWithMetaData(GameModule, type.getSymbol().getName(), modulePath, baseTypeUClass, 0, 0, uemeta.compileClassMetaData(type));
                ...
                type.symbol.valueDeclaration.forEachChild(x => {
                    if (ts.isMethodDeclaration(x) && !manualSkip(x)) {
                        ...
                        if (isStatic && !lsFunctionLibrary) {
                            WarnMessage(sourceFilePath, `do not support static function [${x.name.getText()}]`);
                            return;
                        }
                        if (!isStatic && lsFunctionLibrary) {
                            WarnMessage(sourceFilePath, `do not support non-static function [${x.name.getText()}] in BlueprintFunctionLibrary`)
                            return;
                        }
                        if (x.name.getText() === 'ReceiveInit') {
                            if (baseTypeUClass == UE.GameInstance.StaticClass() || baseTypeUClass.IsChildOf(UE.GameInstance.StaticClass())) {
                                WarnMessage(sourceFilePath, `do not support override GameInstance.ReceiveInit in ${type.getSymbol().getName()}`)
                                return;
                            }
                        }
                        ...
                        let isStatic = !!(ts.getCombinedModifierFlags(x) & ts.ModifierFlags.Static);
                        if (isStatic) {
                            WarnMessage(sourceFilePath, "static property:" + x.name.getText() + ' not support')
                            return;
                        }
                        properties.push(checker.getSymbolAtLocation(x.name));
                    }
                ...
                let defaultPropertis = UE.NewMap(UE.BuiltinName, UE.BuiltinString);
                properties
                    ...
                    .forEach((symbol) => {
                        if (ts.isMethodDeclaration(symbol.valueDeclaration!)) {
                            ...
                            if (!signatures) {
                                WarnMessage(sourceFilePath, `can not find signature for ${symbol.getName()} `)
                                return;
                            }
                            if (signatures.length != 1) {
                                WarnMessage(sourceFilePath, `find more than one signature for ${symbol.getName()} `)
                                return;
                            }
                            ...
                            for (var i = 0; i < signature.parameters.length; i++) {
                                ...
                                if (!paramPinType) {
                                    WarnMessage(sourceFilePath, symbol.getName() + " of " + checker.typeToString(type) + " has not supported parameter!")
                                    bp.ClearParameter();
                                    return;
                                }
                            ...
                            //let text = symbol.valueDeclaration.decorators != null ? symbol.valueDeclaration.decorators[0].getFullText:""
                            //console.log(symbol.getName() + " expression:" + text)
                            ...
                            if (symbol.valueDeclaration.type && (ts.SyntaxKind.VoidKeyword === symbol.valueDeclaration.type.kind)) {
                                ...
                                if (!resultPinType) {
                                    WarnMessage(sourceFilePath, symbol.getName() + " of " + checker.typeToString(type) + " has not supported return type!")
                                    bp.ClearParameter();
                                    return;
                                }
                            ...
                            if (!propPinType) {
                                WarnMessage(sourceFilePath, symbol.getName() + " of " + checker.typeToString(type) + " not support!")
                            } else {
                                let defaultValue = tryGetAnnotation(symbol.valueDeclaration, "defaultValue", true);
                                if (defaultValue != null) {
                                    defaultPropertis.Add(symbol.getName(), defaultValue);
                                }

                                let bAutoInst = tryGetAnnotation(symbol.valueDeclaration, "AutoInst", true);
                                //console.log("bAutoInst:" + bAutoInst);
                                ...
                                if (symbol.valueDeclaration && symbol.valueDeclaration.decorators) {
                                    ...
                                    symbol.valueDeclaration.decorators.forEach((decorator) => {
                                        let expression = decorator.expression;
                                        //console.log("expression:" + expression.getFullText())
                                        if (ts.isCallExpression(expression)) {
                                            ...
                                            if (expression.expression.getFullText().endsWith("uproperty.defaultValue")) {
                                                expression.arguments.forEach((value) => {
                                                    defaultPropertis.Add(symbol.getName(), value.getFullText().slice(1, -1));
                                                });
                                            }
                                        }
                                    });
                                ...
                                bp.AddMemberVariableWithMetaData(symbol.getName(), propPinType.pinType, propPinType.pinValueType, Number(flags & 0xffffffffn), Number(flags >> 32n), cond, bAutoInst != undefined, uemeta.compilePropertyMetaData(symbol));
                            }
                        }
                    });
                ...
                bp.SetupAttachments(sourceFilePath, attachments);
                bp.HasConstructor = hasConstructor;
                bp.Save();

                if (bp.SetPropertyDefaultValue(defaultPropertis)) {
                    bp.Save();
                }
            }
...
try {
    watch(customSystem.getCurrentDirectory() + "tsconfig.json", UE.FileSystemOperation.GetAllPluginInfos());
}
catch (e) {
    UE.TSCompilerErrorReportSubsystem.CleanupErrorReportFile("CodeAnalyze.ts");
    UE.TSCompilerErrorReportSubsystem.AddErrorReport("CodeAnalyze.ts", UE.ETSCompileErrorLevel.Error, e.message);
    UE.TSCompilerErrorReportSubsystem.ShowErrorReport();
}
