...
function readAndParseConfigFile(configFilePath, InfoListOfPlugins) {
    let readResult = ts.readConfigFile(configFilePath, customSystem.readFile);
    let ListOfInclude = new Array;
...
function watch(configFilePath, InfoListOfPlugins) {
    ...
    function onSourceFileAddOrChange(sourceFilePath, reload, program, doEmitJs = true, doEmitBP = true) {
        ...
        if (sourceFile) {
                        ...
                        emitOutput.outputFiles.forEach(output => {
                            ...
                            if (output.name.endsWith(".js") || output.name.endsWith(".mjs")) {
                                ...
                                for (let index = 0; index < InfoListOfPlugins.Num(); index++) {
                                    const plugin = InfoListOfPlugins.Get(index);
            ...
            function getUClassOfType(type) {
                ...
                if (moduleNames.length > 0 && moduleNames[0] == 'ue') {
                    ...
                    if (options.outDir && sourceFileName.startsWith(options.outDir)) {
                        ...
                        bp.LoadOrCreate(GameModule, type.getSymbol().getName(), modulePath, baseTypeUClass, 0, 0);
                        bp.Save();
            ...
            function onBlueprintTypeAddOrChange(baseTypeUClass, type, GameModule, modulePath, sourceFilePath) {
